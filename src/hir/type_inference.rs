...
pub fn inference_in_hir(hir: &mut Hir) {
    inference_in_region(&hir.write().body);
}

pub fn inference_in_region(region: &Arc<RwLock<Region>>) {
    region.write().unwrap().blocks.iter().for_each(|b| inference_in_block(&b));
}
...
pub fn inference_in_expr(expr: &Arc<RwLock<Expression>>) -> Type {
    let ty = match &expr.read().unwrap().body {
        ...
        ExpressionBody::Function{name, args, body, retern_type} => {
            ...
            body.write().unwrap().blocks.iter().for_each(|b| inference_in_block(b));
